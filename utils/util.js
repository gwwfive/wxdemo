...
const compareTime = function (dataTime1, dataTime2) {
  
  if ((typeof dataTime1) == "string" && (typeof dataTime2) == 'string') {
    return ((new Date(dataTime1.replace(/-/g, "\/"))) > (new Date(dataTime2.replace(/-/g, "\/"))));
  } else if ((typeof dataTime1) == "object" && (typeof dataTime2) == 'object') {
    return dataTime1 > dataTime2;
  } else if ((typeof dataTime1) == "object" && (typeof dataTime2) == 'string') {
    return dataTime1 > (new Date(dataTime2.replace(/-/g, "\/")));
  }
...
// 上传资源
var uploadFile = function (resPath, ucallBack) {
  var cos = new COS({
    getAuthorization: function (params, callback) {//获取签名 必填参数
      ...
      // wx.request({
      //   method:'GET',
      //   url: 'https://www.baidu.com/authSign',
      //   data: {
  ...
  var requestCallback = function (err, data) {
    ...
    if (err && err.error) {
      ...
      console.log('url:' + 'https://yourbucket-youracccount.file.myqcloud.com/' + Key)
      ucallBack('https://yourbucket-youraccount.file.myqcloud.com/' + Key)
      // 
    }
  };
  cos.postObject({
